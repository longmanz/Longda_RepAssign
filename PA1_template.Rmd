...
As above function shows, `r NA.num` NAs exist in the dataset.
...
```
So, the mean=`r qu2.mean`, and the median=`r qu2.median`.
...
```
And we can see there are `r sum_NA` NAs.
...
```
So the new mean is `r mean.new`, and teh new median is `r median.new`. In this case, we can see that the new mean is the same as the original one, but the median is now equal to the mean (It's obvious since all the NAs are filled with the mean). 
